...
protocol CameraFlutterApiProtocol {
  ...
  func onShotTypeUpdated(viewId viewIdArg: Int64, shotType shotTypeArg: String, confidence confidenceArg: Double, completion: @escaping (Result<Void, PigeonError>) -> Void)
}
class CameraFlutterApi: CameraFlutterApiProtocol {
  ...
  func onShotTypeUpdated(viewId viewIdArg: Int64, shotType shotTypeArg: String, confidence confidenceArg: Double, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.video_camera.CameraFlutterApi.onShotTypeUpdated\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage([viewIdArg, shotTypeArg, confidenceArg] as [Any?]) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
        return
      }
